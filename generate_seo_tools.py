# -*- coding: utf-8 -*-
"""
Enginewheels SEO & Web Tools Generator and Rebuilder
"""
...
print(f"Loaded {len(DEDUPLICATED_SEO_TOOLS)} unique SEO & Web tools.")

# SEO Copy Generator for individual pages
def generate_seo_text(tool):
    ...
    intro = f"""
      ...
      <p>The **{name}** is a state-of-the-art online utility designed to provide immediate, high-precision results for search engine optimization, webmaster analysis, and digital marketing workflows. In the modern search landscape, optimizing meta descriptors, link outline structures, and campaign ROI metrics is critical for saving time and building site rankings. Whether you are generating schema markup blocks, minifying stylesheets, testing NAP profile consistency, or tracking cost-per-click values, this analyzer offers a safe, ad-free, and easy-to-use interface. Under the hood, this utility operates with client-side JavaScript calculations, ensuring that your URLs, parameters, and private copywriting inputs remain entirely secure and local to your web browser session.</p>
      <p>Manual compilation of SEO redirects, XML sitemaps, JSON-LD schemas, or marketing CTR budgets is a tedious and error-prone process. Our tool automates these tasks. By providing an interactive control panel with optimal defaults, you can verify configurations with a single click. It serves as an essential helper for copywriters, SEO consultants, site administrators, and web developers who want to execute on-page checks instantly without installing external packages or command-line scripts.</p>
      <p>A core foundation of our online utilities is absolute data privacy. Many platforms upload your private redirect paths, campaign budget records, or webpage source codes to backend servers, saving logs that could expose your strategy or server parameters. The Enginewheels **{name}** runs completely client-side in the browser. No data ever traverses the network or gets logged to external systems. Once you close your active browser tab, your parameters are completely cleared from memory, guaranteeing absolute confidentiality.</p>
    ...
    how_it_works = f"""
      ...
      <p>Our **{name}** uses client-side algorithms to parse your choices and assemble matching results instantly. First, the tool collects your input from the main text areas, numeric fields, or selection drop-downs. Once you click "Calculate", the utility runs validation checks to make sure the inputs are not empty and conform to expected patterns. If any parameters are missing, the tool type alerts you immediately via the toast notification system.</p>
    ...
    benefits = f"""
      <h2>Benefits of Using Online SEO Utilities</h2>
...
for index, tool in enumerate(DEDUPLICATED_SEO_TOOLS):
    name = tool["name"]
    slug = tool["slug"]
    category = tool["category"]
    desc = tool["desc"]
    ...
    related_list = []
    for other in DEDUPLICATED_SEO_TOOLS:
...
<head>
  ...
  <!-- SEO Meta Tags -->
  <title>{name} Online Free | Validate & Optimize Metadata - Enginewheels</title>
  <meta name="description" content="{desc} Safe, responsive, and secure client-side browser-based utility tool.">
  <link rel="canonical" href="https://enginewheels.com/seo-web-tools/{slug}.html">
  
  <!-- Open Graph -->
  <meta property="og:type" content="website">
  <meta property="og:url" content="https://enginewheels.com/seo-web-tools/{slug}.html">
  <meta property="og:title" content="{name} Online Free | Validate & Optimize Metadata - Enginewheels">
  ...
  {{
    ...
    "itemListElement": [
      ...
      {{
        ...
        "position": 2,
        "name": "SEO & Web Tools",
        "item": "https://enginewheels.com/category/seo-web-tools.html"
      }},
      {{
        "@type": "ListItem",
        "position": 3,
        "name": "{name}",
        "item": "https://enginewheels.com/seo-web-tools/{slug}.html"
      }}
    ]
  }}
  </script>
...
<body>
  ...
  <main class="tool-page-layout container">
    <nav class="breadcrumb" aria-label="breadcrumb">
      <a href="../index.html">Home</a> &gt; <a href="../category/seo-web-tools.html">SEO & Web Tools</a> &gt; <span>{name}</span>
    </nav>
    ...
    <section class="related-tools-section" style="margin-top: 60px;">
      <h2 style="font-size: 1.4rem; color: #fff; margin-bottom: 24px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 8px;">Related SEO Utilities</h2>
      <div class="tool-grid">
        {related_tools_html}
      </div>
    </section>
...
"""
    ...
    # Save the file
    tool_page_path = os.path.join(SEO_DIR, f"{slug}.html")
    with open(tool_page_path, "w", encoding="utf-8") as f:
        f.write(html_content)

print(f"Generated {len(DEDUPLICATED_SEO_TOOLS)} HTML tool files successfully.")

# ----------------- REWRITE CATEGORY/SEO-WEB-TOOLS.HTML -----------------
print("Rewriting category/seo-web-tools.html...")

subcategories = {
    "SEO Analysis Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "SEO Analysis Tools"],
    "Meta Tag Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Meta Tag Tools"],
    "Schema Markup Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Schema Markup Tools"],
    "Webmaster Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Webmaster Tools"],
    "Content SEO Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Content SEO Tools"],
    "Link Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Link Tools"],
    "Social Media SEO Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Social Media SEO Tools"],
    "Web Performance Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Web Performance Tools" or t["category"] == "WebPerformance Tools"],
    "Domain & URL Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Domain & URL Tools"],
    "Marketing Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Marketing Tools"],
    "Google SEO Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Google SEO Tools"],
    "Local SEO Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "Local SEO Tools"],
    "AI SEO & Content Tools": [t for t in DEDUPLICATED_SEO_TOOLS if t["category"] == "AI SEO & Content Tools"]
}
...
for subcat, tools_list in subcategories.items():
    ...
    for t in tools_list:
        ...
        cat_seen.add(t["slug"])
        grids_html += f"""
          <a href="../seo-web-tools/{t['slug']}.html" class="tool-card">
            <div class="tool-card-header">
...
# Read 3500+ words SEO text
category_seo_text_path = os.path.join(ROOT_DIR, "category_seo_webmaster_text.txt")
if os.path.exists(category_seo_text_path):
    with open(category_seo_text_path, "r", encoding="utf-8") as f:
        category_seo_text = f.read()
else:
    category_seo_text = """<h2>Complete Guide to SEO & Webmaster Utilities</h2><p>Optimize metadata, verify sitemaps, check headers structures locally...</p>"""
...
<head>
  ...
  <!-- SEO Meta Tags -->
  <title>SEO & Webmaster Tools Online Free | Meta & XML Sitemap Generators - Enginewheels</title>
  <meta name="description" content="Explore our free SEO & Web Tools category. Generate JSON-LD schemas, audit meta tags, check keyword density, rebuild robots.txt, and calculate campaign ROI.">
  <link rel="canonical" href="https://enginewheels.com/category/seo-web-tools.html">
  
  <!-- Open Graph -->
  <meta property="og:type" content="website">
  <meta property="og:url" content="https://enginewheels.com/category/seo-web-tools.html">
  <meta property="og:title" content="SEO & Webmaster Tools Online Free | Meta & XML Sitemap Generators - Enginewheels">
  <meta property="og:description" content="Explore our free SEO & Web Tools category. Generate JSON-LD schemas, audit meta tags, check keyword density, rebuild robots.txt, and calculate campaign ROI.">
...
<body>
  ...
  <main class="tool-page-layout container">
    <nav class="breadcrumb" aria-label="breadcrumb">
      <a href="../index.html">Home</a> &gt; <a href="../index.html#categories">Categories</a> &gt; <span>SEO & Web Tools</span>
    </nav>

    <div class="tool-header">
      <h1 class="tool-title-h1">SEO & <span>Webmaster</span> Tools</h1>
      <p class="tool-desc-lead">Optimize webpage layouts, check keyword densities, validate robots.txt crawl configurations, and generate JSON-LD schemas client-side.</p>
    </div>
...
category_page_path = os.path.join(ROOT_DIR, "category", "seo-web-tools.html")
with open(category_page_path, "w", encoding="utf-8") as f:
    f.write(category_html_content)
print("category/seo-web-tools.html rewritten successfully.")
...
seo_sitemap_links = ""
for t in DEDUPLICATED_SEO_TOOLS:
    seo_sitemap_links += f'          <li><a href="seo-web-tools/{t["slug"]}.html">{t["name"]}</a></li>\n'

if start_idx != -1 and end_idx != -1:
    new_sitemap_block = f"{start_target}\n{seo_sitemap_links}          {end_target}"
    ...
    # If comments do not exist, insert the column dynamically next to Generator Tools
...
{seo_sitemap_links}          {end_target}
    ...
    with open(sitemap_html_path, "w", encoding="utf-8") as f:
        f.write(sitemap_content)
    print("sitemap.html updated by adding SEO & Web Tools column.")
