# -*- coding: utf-8 -*-
"""
Auditor and Verifier for Enginewheels Security Tools
"""
...
SEC_TOOLS_DB = []
SEC_TOOLS_DB.extend(PASSWORD_TOOLS)
SEC_TOOLS_DB.extend(HASH_GENERATOR_TOOLS)
SEC_TOOLS_DB.extend(ENCODING_DECODING_TOOLS)
SEC_TOOLS_DB.extend(TOKEN_KEY_GENERATOR_TOOLS)
SEC_TOOLS_DB.extend(ENCRYPTION_TOOLS)
SEC_TOOLS_DB.extend(SECURITY_ANALYSIS_TOOLS)
SEC_TOOLS_DB.extend(NETWORK_SECURITY_TOOLS)
SEC_TOOLS_DB.extend(WEB_SECURITY_TOOLS)
SEC_TOOLS_DB.extend(DEVELOPER_SECURITY_TOOLS)
SEC_TOOLS_DB.extend(PRIVACY_TOOLS)
SEC_TOOLS_DB.extend(CRYPTOGRAPHY_TOOLS)

# Deduplicate by slug
DEDUPLICATED_SECURITY_TOOLS = []
seen_slugs = set()
for tool in SEC_TOOLS_DB:
    if tool["slug"] not in seen_slugs:
        seen_slugs.add(tool["slug"])
        DEDUPLICATED_SECURITY_TOOLS.append(tool)

print(f"Loaded {len(DEDUPLICATED_SECURITY_TOOLS)} unique security tools for verification.")
...
for tool in DEDUPLICATED_SECURITY_TOOLS:
    ...
    if "../assets/js/main.js" not in content:
        ...
        total_errors += 1
    if "../assets/js/crypto-js.min.js" not in content:
        print(f"[-] ERROR in {file_name}: Missing crypto-js.min.js script reference")
        total_errors += 1
    if "../assets/js/jsencrypt.min.js" not in content:
        print(f"[-] ERROR in {file_name}: Missing jsencrypt.min.js script reference")
        total_errors += 1
...
if total_errors == 0:
    print("[+] SUCCESS: All generated security tool files are validated and 100% error-free!")
else:
    exit(1)
